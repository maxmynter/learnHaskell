...
prettyPrint size queens = printBoard size (sortQueens queens)

printBoard :: Size -> [Coord] -> String
printBoard size queens = go (1, 1) queens
  where
    go (row, col) []
      | row > size = ""
      | col > size = '\n' : go (nextRow (row, col)) []
...
sortQueens :: [Coord] -> [Coord]
sortQueens = sortOn (\(row, col) -> (row, col))
...
backtrack (q : restQueens) = nextCol (head restQueens) : tail restQueens
...
finish size trial = case step size trial of
  attempt | length attempt == size + 1 -> tail attempt
  attempt -> finish size attempt
