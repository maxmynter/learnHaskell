...
split text = go text ""
  where
    go (t : ts) rest = if t == ' ' then Just (reverse rest, ts) else go ts (t : rest)
    go [] _ = Nothing
...
checkNumber (a, b) = if go a && go b then Just (a, b) else Nothing
  where
    go s = null [c | c <- s, isDigit c]
...
checkCapitals (for@(f : fos), sur@(s : surs)) = if isUpper f && isUpper s then Just (for, sur) else Nothing
...
winner scores player1 player2 =
  lookupScore player1 >>= \score1 ->
    lookupScore player2 >>= \score2 ->
      Just $ if score1 >= score2 then player1 else player2
  where
    lookupScore name = snd <$> find ((== name) . fst) scores
...
safeIndex l idx = if idx < 0 || idx >= length l then Nothing else Just $ l !! idx

selectSum :: (Num a) => [a] -> [Int] -> Maybe a
selectSum xs is = go is 0
  where
    go [] acc = Just acc
    go (i : is) acc = safeIndex xs i >>= (\n -> go is (acc + n))
