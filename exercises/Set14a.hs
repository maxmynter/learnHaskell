...
greetText name = greeting <> truncName <> suffix
  where
    greeting = T.pack "Hello, "
    truncName = T.take 15 name
    suffix = if T.length name > 15 then T.pack "...!" else T.pack "!"
...
shout :: T.Text -> T.Text
shout phrase = T.unwords $ zipWith ($) (cycle [T.toUpper, id]) (T.words phrase)
...
byteRange bs = if B.length bs <= 1 then 0 else B.maximum bs - B.minimum bs
