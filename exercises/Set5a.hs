...
totalPrice (MkShoppingEntry name price qty) = fromIntegral qty * price
...
data Position = PositionUndefined | MkPosition {x :: Int, y :: Int}
...
study (NthYear n) = if n < 7 then NthYear $ 1 + n else Graduated
