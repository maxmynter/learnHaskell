...
isOrdered (Node v (Node vl ll lr) (Node vr rl rr)) = v >= vl && v <= vr && isOrdered (Node vl ll lr) && isOrdered (Node vr rl rr)
isOrdered (Node v (Node vl ll lr) _) = v >= vl && isOrdered (Node vl ll lr)
isOrdered (Node v _ (Node vr rl rr)) = v <= vr && isOrdered (Node vr rl rr)
...
set (s : sts) val (Node v l r) = case s of
  StepL -> Node v (set sts val l) r
  StepR -> Node v l (set sts val r)
...
search a tree = case search' [] tree a of
  Just steps -> Just (reverse steps)
  Nothing -> Nothing

search' :: (Eq a) => [Step] -> Tree a -> a -> Maybe [Step]
search' sts Empty a = Nothing
search' sts (Node v l r) a
  | v == a = Just sts
  | otherwise = case search' (StepL : sts) l a of
      Just path -> Just path
      Nothing -> search' (StepR : sts) r a
